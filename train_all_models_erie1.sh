#!/bin/bash
# Train 6 models on Lake Erie1 using the current dataset layout.
set -e
...
run_one() {
    local model_short=$1
    local model_class=$2
    local model_id="local_train_${model_short}_erie1"
    echo ""
    echo "============================================================"
    echo "  TRAINING: ${model_class} @ erie1   model_id=${model_id}"
    echo "============================================================"
    time python run.py ${COMMON} \
        --model_id "${model_id}" \
        --model "${model_class}" 2>&1 | tail -6
}
...
echo ""
echo "============================================================"
echo "  ALL 6 MODELS DONE @ erie1"
echo "============================================================"
ls -d results/*erie1* 2>/dev/null | head -10
