...
class Dataset_Custom(Dataset):
    ...
    def __read_data__(self):
        ...
        # For long horizons (large pred_len), small datasets can yield val/test splits
        # shorter than pred_len, which makes __len__ negative and crashes DataLoader.
        # Ensure val/test each have at least pred_len points by borrowing from train.
        min_split_len = max(int(self.pred_len), 1)
        if num_vali < min_split_len:
            deficit = min_split_len - num_vali
            num_vali += deficit
            num_train -= deficit
        if num_test < min_split_len:
            deficit = min_split_len - num_test
            num_test += deficit
            num_train -= deficit

        min_train_len = self.seq_len + self.pred_len
        if num_train < min_train_len:
            raise ValueError(
                f"Dataset too short for seq_len={self.seq_len}, pred_len={self.pred_len} "
                f"with train/val/test split (train={num_train}, val={num_vali}, test={num_test}, total={total_len})."
            )
        ...
        # 可选: 对 target 列 (最后一列, after reorder) 做 log1p + p99 clip 抗极端值
        # 仅在 args.chl_log_transform==1 时生效, 不影响其他列
        if getattr(self.args, 'chl_log_transform', 0) == 1:
            data_arr = df_data.values.copy().astype(np.float64)
            tgt_idx = data_arr.shape[1] - 1
            tgt_train = data_arr[border1s[0]:border2s[0], tgt_idx]
            p99 = float(np.percentile(tgt_train, 99))
            tgt_clipped = np.clip(np.maximum(data_arr[:, tgt_idx], 0.0), 0.0, p99)
            data_arr[:, tgt_idx] = np.log1p(tgt_clipped)
            df_data = pd.DataFrame(data_arr, columns=df_data.columns)
            print(f'  [chl_log_transform] target={self.target}: clipped to [0, {p99:.4f}] then log1p applied')

        # D2: 可选 STL 分解，model 预测 residual = chl - trend - season
        # 反推时把 (trend + season) 加回 prediction
        # 在 log_transform 之后做（如果开启），避免极端值破坏 STL
        self._stl_addback = None  # default: no STL
        if getattr(self.args, 'chl_stl', 0) == 1:
            try:
                from statsmodels.tsa.seasonal import STL
            except ImportError:
                print('  [WARN] statsmodels not installed, skipping STL')
            else:
                data_arr = df_data.values.copy().astype(np.float64)
                tgt_idx = data_arr.shape[1] - 1
                tgt_full = data_arr[:, tgt_idx]
                # STL needs no NaN
                tgt_full = pd.Series(tgt_full).interpolate(limit_direction='both').values
                period = int(getattr(self.args, 'chl_stl_period', 24))
                stl_res = STL(tgt_full, period=period, robust=True).fit()
                trend_season = (stl_res.trend + stl_res.seasonal).astype(np.float64)
                residual = (tgt_full - trend_season).astype(np.float64)
                data_arr[:, tgt_idx] = residual
                df_data = pd.DataFrame(data_arr, columns=df_data.columns)
                # 保存 trend+season 用于 inference 时加回 (按 index 对齐)
                self._stl_addback = trend_season
                print(f'  [chl_stl] period={period}, residual std={np.std(residual):.4f} '
                      f'(orig std={np.std(tgt_full):.4f}, shrink={np.std(tgt_full)/max(np.std(residual),1e-6):.1f}x)')
